...
export const main = async (env: PlayEnv) => {
  const { read, print } = env
  const name = await read('What is your name? ')
  print(`Hello, ${name} welcome to the game!`)

  do
    await play(env, name)
  while (await checkContinue(env, name))
...
type PlayEnv = {
  range: Range,
  read: (s: string) => Promise<string>
  print: (s: string) => void
}

// Play one round of the game.  Generate a number and ask the user
// to guess it.
const play = async ({ read, print, range }: PlayEnv, name: string) => {
...
const checkContinue = async ({ read, print }: CheckContinueEnv, name: string) => {
  for(;;) {
